...
export default function PossibleFlood({setFloodLevel}: { setFloodLevel: (level: string) => void }) {
    ...
    useEffect(() => {
        const fetchLocationAndRisk = async () => {
            try {
                ...
                // Default to medium risk if location fails
                setUserFloodRisk({ risk: "Medium", weight: 2, distance: 0 });
                setFloodAltitude({ altitude: 8.0, distance: 0 });
            }
        };
    ...
    const riskColors = userFloodRisk ? getRiskColors(userFloodRisk.risk) : getRiskColors("Medium");

    return (
        <>
            <Page status={"Flood Nearby. Stay Alert"}>
                <View className="w-full px-3 gap-3">
                    <StatusBox status="Possible Flood Nearby" desc="Stay safe. A flood warning has been issued for your area." icon="⚠" />
                    ...
                    <View className="flex-row gap-3">
                        {/* Personal Flood Risk Widget - First widget in the list */}
                        <View 
                        ...
                        >
                            <Text 
                                ...
                                style={{ color: '#3b82f6' }}
                            >
                                {floodAltitude?.altitude.toFixed(1) || "8.0"} ft
                            </Text>
                            <Text className="text-sm text-gray-400 mt-2 opacity-80 text-center">
                                Flood Water Height
                            </Text>
                    ...
                    <View className="flex-row gap-3">
                        <DataBox data="78%" desc="Humidity" />
                        <DataBox data="1.8″" desc="Rainfall (24 hrs)" />
                    </View>
                    
                    <View className="flex-row gap-3">
                        <DataBox data="68°F" desc="Current Temperature" />
                        <DataBox data="78%" desc="Humidity" />
                    </View>
                    
                    <View className="flex-row gap-3">
                        <DataBox data="5.8 ft" desc="Water Level" />
                        <DataBox data="Rising" desc="River Conditions" />
                    </View>
                    ...
                    <Pressable onPress={() => setFloodLevel("yesFlood")}>
                        ...
                        >
                            <Text className="text-red-400 text-base font-semibold text-center">
                                → Next Stage: Active Flood
                            </Text>
                            <Text className="text-gray-400 text-sm text-center mt-1">
                                Test active flooding scenario
                            </Text>
                        </View>
                    </Pressable>
