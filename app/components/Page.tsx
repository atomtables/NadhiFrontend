...
export default function Page({ children, status }) {
    ...
    const columnNumber = () => {
        if (firstGroup.length > 0 && thirdGroup.length > 0) {
            return (
                <>
                    <View className="flex-row w-11/12 gap-4">
                        <Column>{firstGroup}</Column>
                        <Column>{secondGroup}</Column>
                    </View>
                    <View className="w-11/12 flex-row justify-center">
    ...
    return (
        <ScrollView contentContainerStyle={{ flexGrow: 1, alignItems: 'center' }} className="bg-gray-900 py-8">
            <Welcome />
